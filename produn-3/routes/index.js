...
router.post("/cali/:id", isLoggedin, async (req, res) => {
  try {   
    const newCalif = new Recom({
      userId: req.user._id,
      itemId: req.params.id,
      rating: req.body.cali
    });
    await newCalif.save();
    req.flash(
      "success_message",
      `You have registered the qualification`
    );
    res.redirect("/cart");
   } catch (e) {
    console.log(e)
...
router.get("/", (req, res) => {
  ...
  Product.find().then(products => {
    ...
    for (let i = 0; i < products.length; i ++) {
      productChunks.push(products.slice(i));
    }
    if (req.isAuthenticated()) {
      User.findOne({ email: req.user.email }).then(user => {
...
const recoms = [
  new Recom({
    userId: '5e372cb04df4812117db95ab',
    itemId: '5e372c20d7ab5520cef5b347',
    rating: 1
  })
]
